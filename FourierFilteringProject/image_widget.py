...
class ImageWidget(QWidget):
    ...
    def __init__(self, enable_click, parent=0):
        '''
        Constructor
        '''
        ...
        self.opencv_image = np.array([], dtype=np.uint8)
        self.clearBuffers()
    ...
    def updateImage(self, new_image):
        ...
        if new_image.size:
            ...
            self.opencv_image = copy.deepcopy(new_image)
    ...
    def changeImageWidth(self, new_width):
        if new_width > 0:
            self.desired_width = new_width
            self.updateImage(self.opencv_image)
    ...
    def mousePressEvent(self, event):
        ...
        if self.enable_click:
            ...
            self.imageClicked.emit(row, column)
